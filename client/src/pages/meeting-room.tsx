...
export default function MeetingRoom() {
  ...
  const guestLink = `https://gateway.legal/meet/guest/${meetingId || "demo"}`;
  ...
  // Only enable when mic is on and we're in a meeting
  ...
  useEffect(() => {
    if (meetingId && !localParticipantId && currentUser) {
  ...
  }, [meetingId, localParticipantId, currentUser, joinMeeting, leaveMeeting]);
  ...
  }, [isMicOn, localParticipantId]); // Intentionally omit toggleMic and participants to avoid loops
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [isVideoOn, localParticipantId]); // Intentionally omit toggleVideo and participants to avoid loops
  ...
  const handleEndCall = async () => {
    if (meeting) {
      // Get full transcript before ending
